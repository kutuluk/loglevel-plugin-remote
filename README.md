...
* **json** (boolean) - when set to true, it sends messages in json format:
...
* **timestamp** (function) - a function that returns a timestamp and used when messages sending in json format. By default, it returns the time in the ISO format (see [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601))

* **handle** (string) - the name of the property with which the plugin is managed after using the apply method. By default is `remote` and methods are called like `loglevel.plugins.remote.methodName()`.

After successfully assigning the plug-in, loglevel will add the property loglevel.plugins.remote, with which you can call the following methods:
